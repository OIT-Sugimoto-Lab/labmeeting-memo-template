...
%%
%%
%% 2026-04-07 Yasuhiro Sugimoto <yasuhiro.sugimoto@oit.ac.jp>
%% * 大工大杉本研（ダイナミカルシステム研）研究会資料用に修正
%%   sugimotolab.styを使用するよう変更
%%
%% 2026-04-08 Yasuhiro Sugimoto <yasuhiro.sugimoto@oit.ac.jp>
%% * LuaLaTeX対応に変更
%%   plautopatch, pxjahyperを削除，hyperrefのdvipdfmxオプションを削除
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\RequirePackage[l2tabu, orthodox]{nag}%おまじない

\documentclass{jlreq}%LuaLaTeXはエンジン自動検出のためオプション不要
\usepackage{sugimotolab}%使用前にstyファイル内を一度目を通しておくこと
\usepackage{epic,eepic}
\usepackage{graphicx}%LuaLaTeXはドライバ指定不要
...
%\usepackage{txfonts} % txfontsはLuaLaTeXのTUエンコーディング非対応のため使用不可
\usepackage{newtxtext} % txfontsの後継，LuaLaTeX対応
\usepackage{newtxmath} % txfontsの後継，LuaLaTeX対応（数式フォント，AMSシンボルを内包するためamssymbは不要）
\numberwithin{equation}{section}%式がたくさん出る人はこちらをどうぞ，3章の5式目とかがeq.(3.5)とかになります


\usepackage{hyperref}%ここら辺は本来いりませんハイパーリンク用です（LuaLaTeXはドライバ指定不要）
...
% キーワード
\keywords{研究会資料,ダイナミカルシステム研,大阪工業大学}
%
...
しかし後者の方法は，sugimotolab.styが必要です．より一般的な書き方は前者なので，前者を使うのが良いでしょう．
